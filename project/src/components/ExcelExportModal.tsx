...
const ExcelExportModal: React.FC<ExcelExportModalProps> = ({
  ...
  const availableColumns = useMemo(() => {
    ...
    // Get all unique fields from the actual product data
    ...
    try {
      ...
      const excelData = dataToExport.map(product => {
        ...
        selectedColumns.forEach(columnKey => {
          ...
          row[label] = formatCellValue(product[columnKey], columnKey);
        });
        return row;
      });
      ...
      const colWidths = selectedColumns.map(columnKey => {
        ...
        const maxLength = Math.max(
          ...
          ...dataToExport.map(product => {
            const value = formatCellValue(product[columnKey], columnKey);
      ...
      if (includeStats) {
        const statsData = [
          { Métrica: 'Total de Implementos', Valor: products.length },
          { Métrica: 'Implementos Exportados', Valor: dataToExport.length },
          { Métrica: 'Columnas Incluidas', Valor: selectedColumns.length },
          { Métrica: 'Fecha de Exportación', Valor: new Date().toLocaleString('es-ES') },
          { Métrica: 'Tipo de Exportación', Valor: exportFiltered ? 'Filtrados' : 'Todos' }
        ];
        ...
        // Add branch breakdown if sucursal column is selected
        if (selectedColumns.includes('sucursal')) {
          ...
          Object.entries(branchCounts).forEach(([branch, count]) => {
            statsData.push({ Métrica: `Sucursal: ${branch}`, Valor: count });
          });
        }
      ...
      // Create report entry
      await reportService.createDataExportReport('PRODUCT', dataToExport, 'XLSX');
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] flex flex-col border border-gray-100">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">Exportar a Excel</h2>
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <div className="text-sm text-gray-600 space-y-1">
                ...
                <p><strong>Hojas del archivo:</strong> 
                  Implementos{includeStats ? ', Estadísticas' : ''}, Información de Columnas
                </p>
