...
interface MachinePDFExportModalProps {
  isOpen: boolean;
  onClose: () => void;
  machines: Machine[];
...
const MachinePDFExportModal: React.FC<MachinePDFExportModalProps> = ({
  isOpen,
  onClose,
  machines,
  filteredMachines
  ...
  const [colorTheme, setColorTheme] = useState<'blue' | 'green' | 'purple' | 'gray'>('green');
  ...
  const formatCellValue = (value: any, columnKey: string) => {
    ...
    switch (column?.type) {
      case 'number':
        if (['orderPrice', 'totalPerUnit', 'totalAmountUSD', 'nc'].includes(columnKey)) {
          return `$${(typeof value === 'number' ? value : parseFloat(value) || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}`;
        }
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      const title = 'Reporte de Máquinas';
      ...
      if (exportFiltered) {
        ...
        const filterInfo = `Datos filtrados (${filteredMachines.length} de ${machines.length} registros)`;
      ...
      const tableData = dataToExport.map(machine => 
        selectedColumns.map(key => formatCellValue(machine[key], key))
      );
      ...
      if (includeStats) {
        ...
        // General statistics
        const totalValue = dataToExport.reduce((sum, machine) => sum + (machine.totalAmountUSD || 0), 0);
        const totalNC = dataToExport.reduce((sum, machine) => sum + (machine.nc || 0), 0);

        const generalStats = [
          ['Total de Máquinas', machines.length.toString()],
          ['Máquinas Exportadas', dataToExport.length.toString()],
          ...
          ['Total NC', `$${totalNC.toLocaleString('en-US', { minimumFractionDigits: 2 })}`],
          ['Fecha de Exportación', new Date().toLocaleString('es-ES')],
          ['Tipo de Exportación', exportFiltered ? 'Filtradas' : 'Todas'],
        ...
        // Location breakdown if ubicacion column is selected
      ...
      // Create report entry
      await reportService.createDataExportReport('MACHINE', dataToExport, 'PDF');
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-5xl w-full max-h-[90vh] flex flex-col border border-gray-100">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">Exportar Máquinas a PDF</h2>
...
export default MachinePDFExportModal;